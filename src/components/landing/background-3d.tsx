...
function Particles({ count = 200 }: { count?: number }) {
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (colorChoice < 0.4) {
        colors[i3] = 0.4; colors[i3 + 1] = 0.4; colors[i3 + 2] = 0.95;
      } else if (colorChoice < 0.7) {
        colors[i3] = 0.55; colors[i3 + 1] = 0.25; colors[i3 + 2] = 0.9;
      } else {
        colors[i3] = 0.2; colors[i3 + 1] = 0.7; colors[i3 + 2] = 0.85;
      }
  ...
  return (
    <points ref={mesh}>
      <bufferGeometry>
        ...
        <bufferAttribute attach="attributes-size" args={[particles.sizes, 1]} count={particles.sizes.length} />
      </bufferGeometry>
...
function FloatingOrb({ position, scale, color, speed = 1 }: { position: [number, number, number]; scale: number; color: string; speed?: number; }) {
  ...
  return (
    <mesh ref={mesh} position={position} scale={scale}>
      ...
      <MeshDistortMaterial color={color} transparent opacity={0.15} distort={0.4} speed={2} roughness={0.2} />
    </mesh>
  );
}

function FloatingTorus({ position, scale, color }: { position: [number, number, number]; scale: number; color: string; }) {
  const mesh = useRef<THREE.Mesh>(null);
  const { pointer } = useThree();
